...
from Functions.helpFunc import calculate_courier_earnings, calculate_courier_rating
...
def get_my_profile(current_user = Depends(require_courier), db: Session = Depends(get_db)):
    ...
    courier = db.query(CourierDB).filter(CourierDB.id_user == user_id).first()
    if not courier:
        raise HTTPException(404, "Курьер не найден")

    earnings = calculate_courier_earnings(courier.id_courier, courier.courier_type, db)
    rating = calculate_courier_rating(courier.id_courier, db)
...
def take_order(order_id: int, current_user=Depends(require_courier), db: Session = Depends(get_db)):
    ...
    if round(order.weight, 2) > max_load:
    ...
    my_orders = db.query(OrderDB).filter(OrderDB.id_courier == courier_id,OrderDB.status == "assigned").all()

    current_total_weight = sum(o.weight for o in my_orders)


    # проверка грузоподъёмности
    if round(current_total_weight + order.weight, 2) > max_load:
...
def get_my_orders(current_user = Depends(require_courier),db: Session = Depends(get_db)):
    ...
    # находим курьера по id_user
    courier = db.query(CourierDB).filter(CourierDB.id_user == user_id).first()
    if not courier:
        return {"orders": [], "total": 0}  #если курьер не найден, возвращаем пустой список
...
def complete_order(order_id: int, current_user = Depends(require_courier), db: Session = Depends(get_db)):
    ...
    if order.id_courier != courier_id:
...
def get_my_rating(current_user=Depends(require_courier), db: Session = Depends(get_db)):
    ...
    #если нет завершённых заказов возвращаем 0
    if rating is None:
        return {"rating": 0}
